...
###############################
PM1_DataOutput <- stan("Stan_ProductivityModel1_Autoregressive.stan", data=stan_data, chains=3, iter=5000, control=list(max_treedepth=12) )
...
#PM3_DataOutput <- stan("Stan_ProductivityModel3_ThinFilm.stan", data=stan_data, chains=3, iter=5000, control=list(max_treedepth=12) )

# View on shiny
launch_shinystan(PM2_DataOutput) ## Click explore tab when new window opens
# Save Output
saveRDS(PM1_DataOutput,"PM1_DataOutput.rds")
saveRDS(PM2_DataOutput,"PM2_DataOutput.rds")
...
PM3_medpar <- mechN_extract_medians(rstan::extract(PM3_DataOutput, c("alpha","gamma","s","c","N","P","pred_GPP","sig_p")))
...
## PM1 Simulation
pars1<-rstan::extract(PM1_DataOutput, c("phi","alpha","beta","sig_p"))
...
## For every day extract mean and sd
mean_simmat1 <- apply(simmat1, 1, function(x) mean(x))
sd_simmat1 <- apply(simmat1, 1, function(x) sd(x))

## PM2 Simulation
pars2<-rstan::extract(PM2_DataOutput, c("r","K","s","c","B","P","pred_GPP","sig_p"))
...
sd_simmat2 <- apply(simmat2, 1, function(x) sd(x))

## Compile Stan data
stan_simdat_compile <- function(x, mean_simGPP, sd_simGPP){
  data <- list(Ndays=length(x$GPP), light = x$light_rel, GPP = mean_simGPP, tQ = x$tQ, GPP_sd = sd_simGPP)
  return(data)
}

stan_simdat1 <- stan_simdat_compile(df, mean_simmat1, sd_simmat1)
stan_simdat2 <- stan_simdat_compile(df, mean_simmat2, sd_simmat2)
...
launch_shinystan(simPM1_DataOutput)
...
## Source parameter recovery functions
